...
if (isset($_POST['changer_psd'])) {
    ...
    $sql = "SELECT * FROM employee WHERE id_employee = '{$_SESSION['prof_id']}' AND password = '$psd1'";
    $sql = mysqli_query($connect, $sql);
    if (mysqli_num_rows($sql) > 0) {
        $dt = mysqli_fetch_object($sql);

        if ($psd2 == $psd3) {
            $sql = "UPDATE employee SET password = '$psd2' WHERE id_employee = '{$_SESSION['prof_id']}'";
